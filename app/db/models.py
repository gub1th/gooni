...
class Note(Base):
    ...
    #
    # Every other flag (pinned, public, tags) is preserved verbatim across
    # archive/unarchive, so restoring is a single bit flip.
...
class FocusSession(Base):
    ...
    **A stopped session still writes the entry.** `focus_session_service.stop`
    ...
    #: The narrated summary of what actually happened this session, generated by
    #: ONE LLM call over the screen evidence + camera presence once the shell has
    #: finished posting the window (see `screen_evidence_service.summarize`).
    #: NULL until evidence lands and the summary runs — a session with no
    #: Screenpipe running simply never gets one, which reads as "no screen data"
    #: rather than an error. Text, free-form prose.
    screen_summary = Column(Text, nullable=True)
    #: The model's on-task estimate (0-100) vs this session's stated task. The
    #: one semantic judgement a deterministic ranker cannot make. NULL when
    #: unmeasured — never 0, which would read as "did nothing" (the same
    #: None-not-zero rule the focus score follows).
    on_task_pct = Column(Integer, nullable=True)
    #: When the summary was generated, so a stale one can say so and a re-run is
...
class SessionScreenFrame(Base):
    """One captured moment of screen (+ its text) inside a focus session.

    THE ingest target for Screenpipe evidence. Screenpipe runs locally and owns
    the raw capture (event-driven screenshot + macOS AX tree + Apple Vision OCR
    → its own SQLite in ~/.screenpipe). Gooni does NOT re-capture; the desktop
    shell reads Screenpipe's DB for exactly a stopped session's [start, stop)
    window and POSTs the frames here. See `screen_evidence_service`.

    WHY SESSION-SCOPED, not 24/7. Screenpipe stores ~1GB/day, most of it an idle
    screen nobody will ever scrub back to. A focus session is the ONLY window
    worth replaying, so only those frames land in the cloud — the same
    bounded-window discipline `focus_session_activity` and `focus_attribution`
    already follow, here for storage cost rather than for attribution honesty.

    TEXT is Phase 1, the IMAGE is Phase 2, and the split is deliberate:
      · `text` (the flattened AX/OCR content of the frame) is what the summary
        LLM reads and what makes the timeline searchable — it ships always.
      · `r2_key` is NULL until the frame's JPEG is uploaded to R2 (Phase 2). A
        frame with text and no image is a complete, useful row — the summary and
        the app/site band need no picture. The image only buys the visual
        scrubber, so it is additive by construction.

    RAW-ish, but already one step cooked: `app`/`window_name`/`url` come straight
    off Screenpipe's frame row, `text` is its extracted content. No score, no
    judgement here — that is the summary's job (`screen_summary` on the session).
    """

    __tablename__ = "session_screen_frames"

    id = Column(Integer, primary_key=True, index=True)

    # The session this frame belongs to. Indexed because every read is
    # "all frames for session N, in time order". No cascade: a deleted session
    # is rare and its evidence is cleaned explicitly, the same stance the rest
    # of the focus tables take toward a vanished parent.
    session_id = Column(Integer, ForeignKey("focus_sessions.id"), nullable=False, index=True)

    # Naive UTC, like every datetime in this schema. The frame's own capture
    # time from Screenpipe — NOT ingest time, since the shell posts a whole
    # window at once after the session stops.
    ts = Column(DateTime, nullable=False, index=True)

    # Straight off Screenpipe's frame row. `app` is the frontmost app, `url` the
    # browser URL when it's a browser frame (Screenpipe captures it natively),
    # `window_name`/`title` the window/page title.
    app = Column(String, nullable=True)
    window_name = Column(Text, nullable=True)
    url = Column(Text, nullable=True)
    title = Column(Text, nullable=True)

    # The flattened AX/OCR text of the frame — what the summary reads and what
    # makes this searchable. Can be long; Text not String.
    text = Column(Text, nullable=True)

    # Phase 2. The R2 storage key for this frame's JPEG, NULL until uploaded.
    # A key, not a URL, so the public host can change without a rewrite — the
    # serializer builds the URL, the same way attachments do.
    r2_key = Column(String, nullable=True)

    # Idempotency boundary. The shell mints it from Screenpipe's frame id +
    # session, so a re-posted window dedups instead of double-inserting — the
    # same UNIQUE-index dedup the interval sensors and WaProcessedId use.
    client_id = Column(String, nullable=False, unique=True, index=True)
