"""`NoteService` — embedding writes and note search (FTS5 + semantic).

Split out of the old single-file note_service. The class holds the READ
and WRITE primitives for a note's vector; `classify` owns the extraction
pipeline and `sweep` owns when either runs.
"""
...
class NoteService:
    ...
    def update_embedding(self, note_id: int) -> None:
        """Generate and store an embedding for a note, stamping `embedded_at`.

        Opens its own DB session: every caller is off the request path (the
        sweeper's thread, or the manual /classify route's). It shipped with
        this same docstring calling itself a background task while the route
        awaited it inline — so it paid for its own session AND blocked the
        response. The sweeper is what finally made that true again.

        `embedded_at` is stamped only on a SUCCESSFUL write, so a failed
        embedding leaves the note due and the next sweep retries it.
        """
