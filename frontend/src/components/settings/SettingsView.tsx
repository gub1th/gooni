      ...
      <section style={{ marginBottom: 22 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 0 }}>
            ...
            <span style={{ fontSize: 11.5, color: "var(--gooni-faint, #94A3B8)", fontFamily: FONT }}>
              {avatarUrl
...
function StatusDot({ status }: { status: HealthState["status"] }) {
  const color = status === "ok" ? "#30D158" : status === "down" ? ctok.danger : status === "checking" ? "#FFD60A" : ctok.disabled;
  return (
    <span style={{
      ...
      {editingVercel ? (
        <div style={{ border: `1px solid ${ctok.hairline}`, borderRadius: 10, padding: 12, background: ctok.card }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={saveVercelUrl} style={{ ...btn, background: ctok.text, color: ctok.card, border: "none" }}>save</button>
...
function DeploymentCard({ name, url, dashboardUrl, state, onRecheck, onEditUrl, meta }: DeploymentCardProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 11.5, color: "var(--gooni-muted, #6B6B70)",
        fontFamily: "'SF Mono', Menlo, monospace",
...
function GithubLogo() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={ctok.text} xmlns="http://www.w3.org/2000/svg" aria-hidden>
