 ...
 */
function summarize({ config, sidecar, tokenSource }) {
  const problems = [];
...
function buildMenuTemplate({ config, sidecar, tokenSource, launchAtLogin, handlers = {} }) {
  const summary = summarize({ config, sidecar, tokenSource });
  const items = [];
