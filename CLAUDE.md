...
2. The extractor annotates commitment-shaped messages with a glow; Daniel promotes them to Promises with one tap (or dismisses); fitness/measurements log to Trackables in real time
...
- `Memory` — KEPT (Daniel's override). Reconcile dance + cosine retrieval + MCP tools unchanged; the capability prompt block is gone (OBJECT_KINDS is the remaining anti-hallucination anchor).
...
- **Full chat view retired** — `components/ChatView.tsx` + `components/chat/{InputBar,MessageBubble}.tsx` deleted; `?view=chat` + `?conv=`-driven view gone, nav entries pulled (SummonedNav, CollapsedSidebar, QuickNav). The floating `GooniPanel` orb (via `ChatLauncher`/`GooniLayer`) is the surviving chat surface; `useConversationsStore` stays (orb + ambient capture still use it).
...
- **`components/SettingsPanel.tsx`** — General tab: timezone only (the one surviving Settings knob). `SettingsModal` tabs = Profile/Appearance/General/**Widgets** (per-registered-widget on/off toggles → `useWidgetLayoutStore`)/Integrations/Deployments. `components/dashboard/` deleted in the lean sweep (zero importers).
