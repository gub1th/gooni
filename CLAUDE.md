...
- **`components/Dashboard.tsx`** — Single-column dashboard with a top-tier mode toggle. Layout top→bottom: `DashboardHeader` → `ModeToggle` (Today | Build | Pulse, segmented pill; persists under `useDashboardStore` key `gooni-dashboard-v3`). Mode-specific body content: **Today** = TakeTabs → NoteEditor quick-input → inner `TabToggle` (Todos | Focuses) → TodoList or FocusesView → HabitsStrip. The Today layout reacts to `useDashboardStore.composerFocused` — when the embedded NoteEditor gains focus, TakeTabs collapses to 0fr (CSS grid-row trick) and the focuses/todos block dims to 0.15 opacity, putting the writing surface front-and-center w/o a modal. Esc / submit / blur exits focus mode. **Build** = `BuildMode` (Gooni-health 6-axis composite scores). **Pulse** = `PulseMode` (life-stats grid).
...
- **`components/dashboard/ModeToggle.tsx`** — top-tier dashboard mode toggle (Today | Build | Pulse). Border-radius matches the focus / todo cards (12) so the dashboard reads as one coherent layout. No per-mode bg tint — global theme bg (Settings → Appearance) owns the page surface. Active mode persisted in `useDashboardStore` (`gooni-dashboard-v3`).
