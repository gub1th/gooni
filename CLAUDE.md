...
- **`app/services/take_service.py`** — Daily LLM-generated takes. `get_or_generate(db, kind, force=False)` upserts a `GooniTake` row keyed on (today, kind). kind="focus" reads recent notes + active focuses; kind="dev" reads commits/PR titles across tracked repos (last 7d). Empty takes aren't persisted — keeps yesterday's row alive when today's source is empty (no commits, no notes). Per-kind versions live in `PROMPT_VERSIONS` (focus="v2", dev="v3"); a stored row with a stale `prompt_version` is auto-regenerated on next read so prompt bumps roll out without manual cache busts. **Dev take is JSON, not prose** — v3 stores a JSON-stringified array of `{theme, summary}` (max 5 themes, e.g. `[{"theme":"Chat","summary":"…"}]`). `_parse_dev_themes` validates the LLM output + strips ```json fences. Frontend `parseDevTake` (in `services/api.ts`) returns either `{kind:"themes",…}` or `{kind:"text",…}` so legacy v2 paragraph rows still render until next regen.
...
- **`components/Dashboard.tsx`** — Single-column dashboard with a top-tier mode toggle. Layout top→bottom: `DashboardHeader` → `ModeToggle` (Today | Ops | Pulse, segmented pill; persists under `useDashboardStore` key `gooni-dashboard-v3` w/ `migrate` that coerces legacy `activeMode='build'` to `'ops'`). Mode-specific body content: **Today** = TakeTabs → NoteEditor quick-input → inner `TabToggle` (Todos | Focuses) → TodoList or FocusesView → HabitsStrip. The Today layout reacts to `useDashboardStore.composerFocused` — when the embedded NoteEditor gains focus, TakeTabs collapses to 0fr (CSS grid-row trick) and the focuses/todos block dims to 0.15 opacity. Esc / submit / blur exits focus mode. **Ops** = `OpsMode` (Gooni-health cards + CapabilityProfileCard + eval drilldown + backlog kanban + tool-call failures — folded in what Build used to surface). **Pulse** = `PulseMode` (life-stats grid).
- **`components/dashboard/TodoList.tsx`** — Pulls `/todos` (primary + open + done_today buckets). 3-state cycle: empty square → dotted half (doing) → filled check (done); from done, click pops a state-picker. Focus color dot per row (resolves via `utils/focusColors.ts`). Hover row exposes promote-to-primary (crown) + delete (×). PrimaryCard wears a soft yellow halo (stacked box-shadows) + a one-shot conic-gradient race-border animation (`gooni-primary-race`, ~1.1s) that draws around the card perimeter on mount and dissolves into the glow. Keyed on `t.id` so re-promoting another todo re-fires the animation.
- **`components/dashboard/FocusCardsRow.tsx`** — Pulls `/focuses`. 3-col grid above the todo list. Every card sits on a soft blurred colored halo in its own focus color; the highlighted (currently first) card adds a slow-rotating conic-gradient sheen on top (`gooni-focus-glow-spin`, 14s) for a Gemini-ish multi-color glow. Halo is a separate absolutely-positioned layer behind the white card so foreground text stays opaque.
...
- **`components/dashboard/ModeToggle.tsx`** — top-tier dashboard mode toggle (Today | Ops | Pulse). Border-radius matches the focus / todo cards (12) so the dashboard reads as one coherent layout. No per-mode bg tint — global theme bg (Settings → Appearance) owns the page surface. Active mode persisted in `useDashboardStore` (`gooni-dashboard-v3`).
- **`components/dashboard/BuildMode.tsx`** — "Gooni Health" surface. 3-col grid of `HealthCard`s, one per axis (memory / chat / engagement / availability / cost / connectors). Each card click opens `HealthDrillDown` modal w/ per-component breakdown bars. Pulls `/health/scores`; scores recomputed on every mount (60s staleTime). Mounted at the top of OpsMode (was its own Build tab, now folded in).
- **`components/dashboard/OpsMode.tsx`** — "operator's console". Top → bottom: `BuildMode` (health cards, 3-col) + `CapabilityProfileCard` → `EvalSection` (one unrated `EvalSegment` at a time via `fetchEvalSegmentFull`; per-assistant-turn rating row 1/2/3 + optional inline comment textarea via `putMessageRating`; Skip = client-side advance (queued in a local `Set<segmentId>`); Done = `patchEvalSegment({eval_status:"done"})` then refetch + advance) → `BacklogSection` (the kanban `BacklogBoard` rendered inside a 360px fixed-height scroll container + an "open full →" link that navigates to the standalone `/?list=<backlog-id>` route via `useNavigate`) → `FailuresSection` (recent `ToolCallFailure` rows, kept compact).
...
GET  /dashboard/dev-take        → today's dev take. v3 returns a JSON-stringified array of `{theme, summary}` in `take` (themed weekly summary for a head-of-eng audience — 3-5 themes, no PR numbers). Legacy v2 rows return prose; frontend `parseDevTake` handles both. Persisted in `gooni_takes` kind="dev"; ?force=1 regenerates. Auto-regenerates when `prompt_version` doesn't match `PROMPT_VERSIONS["dev"]`.
