...
- **`app/services/orchestrator.py`** — Unified chat handler across all surfaces (web, telegram, whatsapp, imessage). `Orchestrator` singleton. Source defaults to `"web"`; bot channels share a single persistent conversation per source (no gap-based sessioning). Each turn builds a structured trace via `TraceBuilder` and stamps it on `Message.trace`. Per-turn signal extraction (`extract_signals` in `memory_extraction`) emits tone_corrections, feature_requests, soft_promises, and memory candidates in ONE LLM call; each is persisted via its respective service (memory → memory_service, feature → backlog_service, promise → promise_service). Bot-channel turns get four master-prompt blocks: `cadence_block` (Alfred voice rules — 1 bubble default, 2 max, ≤3-word acknowledge when criticized, action-over-preface, no self-flagellating paragraphs), `state_block` (`[your state right now]` — primary todo + open count + done-today + pending promises due ≤24h with slip_count, fixes T1 of eval segment #209 where "Yo" got a scolding guess instead of state-grounded reply), `just_extracted_block` (`[just extracted from this message — already routed, don't re-announce]` — tells the LLM what got routed so it doesn't re-announce / hallucinate), and the cadence rules themselves. The ack helper `_build_ack` (alias `_build_jarvis_ack` retained) composes terse alfred-voice acknowledgements — drops "got it:" / "noted —" prefixes, surfaces slip_count inline as `slip #N`.
- **`app/services/promise_service.py`** — Promise CRUD + lifecycle (pending → kept | broken | abandoned). `create()` infers deadline from time hints via `_infer_due_from_text` (regex over "tonight"/"tomorrow"/"this week"/etc), embeds the utterance, auto-wires an `utters` edge from the source Message + a `supports` edge to the nearest cosine-matched active Focus (threshold 0.75). `slip_count` set on create by cosine-matching against past `broken` promises (threshold 0.80) so re-uttering a chronically-broken pattern surfaces immediately. **Active-pending dedup** (`_find_pending_duplicate`, threshold 0.85): if the utterance cosine-matches an existing pending Promise above the bar, `create()` returns the existing row instead of inserting a duplicate — wires an `utters` edge from the new source message so the re-statement is still recorded. Fixes T4→T5 of segment #209 where re-uttering "find leg doctor" silently piled up rows. `auto_mark_overdue` sweeps stale pending → broken (run by the daily nudge before picking the focus item).
- **`app/services/edge_service.py`** — Generic graph layer over the `edges` table. Module-level helpers (no class): `link()` (idempotent on the 5-tuple), `unlink()`, `links_for(kind_of_node, node_id, edge_kind=None)` (bidirectional traversal — every edge touching the node), `neighbors(direction='out'|'in'|'any')`, `serialize_edge()`. Caller code stays in service layers (promise_service wires `utters`/`supports` at promise create; future entities plug in the same way).
- **`app/services/fly_revive.py`** — Boot-time handshake. `catch_up_orphaned_messages(db)` finds WA conversations where the most-recent message is from Daniel + received within last 24h + has no assistant reply after it, then sends a brief "fly died, back online" apology via `whatsapp_channel.send()` and records it as an assistant Message row. Idempotent automatically: once the apology lands as an assistant row, the conv no longer has an orphan tail. v1 does NOT re-process the orphaned message through the orchestrator — Daniel can re-ask if he wants the actual answer. Wired into `_lifespan` boot in `app/main.py`.
- **`app/services/todo_nudge.py`** — Daily morning digest. Post-promises rewrite: pulls ONE thing to nudge about (priority: pending promise due within 24h → any pending promise → primary todo → most-overdue todo) and asks Daniel conversationally about that single item. No chore-list dump — Daniel called the old multi-item summary "bs". Voice instruction in `DEFAULT_PROMPT` enforces friend-texting cadence; falls back to a static one-liner if the LLM call fails. Sweeps `promise_service.auto_mark_overdue` before picking so dead promises aren't surfaced.
- **`app/tools/`** — chat tool surface exposed to the LLM via OpenAI function-calling. Registry in `app/tools/__init__.py` (~25 tools): memory (save_memory), web (web_search, fetch_url), lists (add_to_list, show_list, check_list_item), notes (search_notes, add_note, find_note, read_note, list_recent_notes), todos (add_todo, list_todos, set_todo_state — 3-state: not_yet|doing|done), focuses (add_focus, list_focuses), habits (log_habit — fuzzy resolves habit name by case-insensitive substring; refuses unknown habits so the LLM can't surprise-create), feature_request, **activity** (read_recent_commits — per-repo subjects across tracked GitHub repos within N days; read_recent_backlog — tickets grouped by status with PR links, fixes T3 of segment #209 where Gooni couldn't recap shipped work), calendar (5). Destructive tools (delete/forget/edit-memory/backlog ops) are deliberately NOT exposed to chat — chat surface is non-destructive by design so SMS typos can't wipe data. Dev-only / Claude-Code-only tools live in `mcp/server.py` and stay there.
