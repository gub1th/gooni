...
- **`app/db/models.py`** — SQLAlchemy models. Grep for fields; high-leverage notes only:
  ...
  - `Promise` — soft commitment uttered in chat ("imma X tonight"). Distinct primitive from Todo (chore) and Focus (long arc). **G3.1 lifecycle: `active → kept|broken`** (auto-broken when `inferred_due` passes). The old `proposed → pending` lock-in split + the `abandoned` terminal are GONE — every utterance lands `active` immediately. `promise_complexity.needs_game_plan` still runs but only stamps `needs_clarification` metadata (drives ack pushback); it no longer gates state. Recurring-shape utterances (per `promise_complexity.is_recurring`) auto-spawn a Habit + `measured_by` edge **at create time** (was the lock-in flip). `slip_count` set on create via cosine match against past broken promises. Cross-links land in `edges` (utters from Message, supports nearest Focus, measured_by Habit when recurring). Manual-add path: `POST /promises {text}` (dashboard PromiseDrawer) routes through the same `promise_service.create`, minus the source-message `utters` edge.
...
GET    /promises?state=active|kept|broken&limit=50 → list w/ deadline-asc sort for active, recency for everything else
POST   /promises                      → { text } → manual add (dashboard PromiseDrawer). Runs promise_service.create → lands active. 400 on empty text.
GET    /promises/pis                  → Promise Integrity Score (weighted aggregate over last 20 resolved: kept +1, broken -1.5). Returns { score: 0-100 | null, sample_size, kept_streak, last_broken_at, last_broken_summary, weights, window }. Null score when sample_size < 3 (small-N noise distortion). Endpoint live but the dashboard PromiseIntegrityCard was removed — no FE consumer currently.
PATCH  /promises/{id}                  → { state: active|kept|broken }. Idempotent — re-sending current state is a no-op. kept/broken stamp resolved_at; reviving to active clears it.
