...
- **`app/services/focus_candidate_service.py`** — Persistence + lifecycle for `FocusCandidate` rows. `persist_run(db, synth_output)` upserts focus-shaped clusters (top-level + sub-cluster focuses) by `cluster_signature`. Re-sight bumps seen_count + refreshes the LLM-classified name/endgoal/reasoning + evidence snapshot; status stays sticky once a candidate is promoted or dismissed. `promote()` creates a real `Focus` row via `focus_service.create` and links via `promoted_focus_id`. `dismiss()` flips status + stamps timestamp. State / noise clusters intentionally not persisted.
...
POST /focus-candidates/run      → runs synth AND persists focus-shaped candidates via `focus_candidate_service.persist_run` (upsert by `cluster_signature`; seen_count bumps on re-sight; status sticky once promoted/dismissed). Same body knobs as the probe. Returns {synth_stats, persisted}.
