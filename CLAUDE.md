...
- **`routes/creative.tsx`** — Experimental 3D scene at `/creative` (standalone shell, no sidebar). React-three-fiber v8 + drei v9 + @react-three/postprocessing v2 canvas (all React 18-compatible pins). Components in `src/components/creative/`. The scene composes: dusk Sky + warm directional light (`Atmosphere.tsx`), drei `<Environment preset="sunset">` for PBR reflections, reflective pond via `three/examples/jsm/objects/Water.js` extended into R3F (`Pond.tsx`), shore ring of instanced low-poly stones + grass tufts at the fog fade (`Shore.tsx`), instanced lily pads w/ slit-cutout ShapeGeometry + per-instance bob/hue/yaw drift (`LilyPads.tsx`), sakura petal canopy via instancedMesh w/ per-petal swing+respawn (`Petals.tsx`), stylized stern-sculling rowboat — GLTF (`/models/rowboat.glb`) w/ primitive fallback that includes a seated hat-wearing rower and an animated stern oar tied to thrust (`Boat.tsx`), drei `<Trail />` wake behind the boat, click-on-water ripple pool (`Ripples.tsx` — `<RippleClickPlane>` is a colorWrite=false plane that captures pointerdown and forwards to a 16-slot expanding-ring pool), 3rd-person follow camera w/ speed-based FOV punch + soft tether back toward origin past r=38, ACES Filmic tone mapping + Bloom + Vignette post-processing (`PostFX.tsx`), drei `<AdaptiveDpr />` + `<AdaptiveEvents />` for interaction-time downsampling, click-gated ambient audio (`AmbientAudio.tsx`), and a `useProgress()`-backed loading bar on the StartOverlay. Controls flow through a module-singleton input state (`useBoatControls.ts`): `useBoatKeyboard()` installs WASD/arrows + R-reset, `MobileJoystick.tsx` writes the same axes from touch (virtual analog stick bottom-center, plus a reset button bottom-right). Assets in `public/{models,textures,audio}/` — texture loader falls back to a procedural normal map if `waternormals.jpg` is absent; GLTF falls back to the primitive boat; missing audio just no-ops. Long-term: may replace `/public` once it earns its keep.
...
- **`components/Dashboard.tsx`** — Greeting + day-streak stat tile (notes-this-week was dropped in the dashboard revamp). Gooni's Take pill (focus take, kind="focus" from `gooni_takes`) above the composer; Dev Take (kind="dev") below the work surfaces. Below the composer: `WhoopStrip` (3 slim cards — Recovery / Sleep / Strain — only renders when Whoop is connected) → `FocusCardsRow` (horizontal cards w/ color rail + X/Y progress, "+ add" creates a committed focus inline) → `TodoList` (primary singleton crowned at top, open todos w/ `doing` floated above `not_yet`, inline create, age tag, cascade-done animation, Done section toggle: Completed today ↔ Dev activity) → `HabitsStrip` (daily binary trackers, one row per habit w/ name + 7-day strip + streak; bottom of dashboard). Public bio editor lives on `/public`, not the dashboard.
