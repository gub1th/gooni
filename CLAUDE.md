...
- **`components/Dashboard.tsx`** — Single-column. Mode toggle (Today|Review|Ops|Stats|TV, key `gooni-dashboard-v3`, migrates `build`→`ops`, `pulse`→`stats`). Today body reacts to `composerFocused`. `TvMode` = read-only ambient display (big todos/promises + reused CutTable/Whoop sections, 5-min auto-refresh) — additive tab, no kiosk route.
